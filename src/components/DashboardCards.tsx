...
export interface DashboardStats {
  totalLojas: number
  promotoresAtivos: number   // ← MUDOU: antes era totalPromotores
  visitasHoje: number        // ← MUDOU: antes era checkinsHoje
  cobertura: number
}
...
export async function getDashboardData(lojaId?: string | null, isAdmin?: boolean): Promise<{
  ...
  try {
    ...
    const promotoresAtivos = promotores?.length || 0
    ...
    const visitasHoje = checkins?.length || 0
